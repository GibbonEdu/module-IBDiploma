    ...
    if ($ibDiplomaStudentID == '') { $page->addError(__('You have not specified a student member.'));
        ...
        if ($result->rowCount() != 1) {
           $page->addError(__('The selected student member does not exist.'));
            ...
            $form = DeleteForm::createForm($session->get('absoluteURL').'/modules/'.$session->get('module')."/student_manage_deleteProcess.php?ibDiplomaStudentID=$ibDiplomaStudentID");
            echo $form->getOutput();
        }
    }
}
