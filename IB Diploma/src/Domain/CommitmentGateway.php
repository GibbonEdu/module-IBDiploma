...
class CommitmentGateway extends QueryableGateway
{
    ...
    public function queryCommitments($criteria, $gibbonSchoolYearID, $gibbonSchoolYearSequenceNumber, $gibbonPersonID) {      
        $query = $this
            ...
            ->cols(['ibDiplomaCASCommitment.*', 'gibbonPerson.gibbonPersonID', 'gibbonStudentEnrolment.gibbonYearGroupID', 'gibbonStudentEnrolment.gibbonFormGroupID', 'ibDiplomaStudentID', 'surname', 'preferredName', 'start.name AS start', 'end.name AS end', 'gibbonYearGroup.nameShort AS yearGroup', 'gibbonFormGroup.nameShort AS formGroup', 'gibbonPersonIDCASAdvisor', 'casStatusSchool'])
